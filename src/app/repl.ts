...
import { resolveOpenAIConfig } from '../config/provider-config';
import { loadProject } from '../config/project-loader';
import { loadProjectContext, ProjectContextSummary } from './project-context';
import { clearSessionState, createSessionState, pushSessionEntry } from './session-state';
...
interface ParsedAction {
  type: 'read' | 'search' | 'list_files' | 'list_workflows' | 'list_agents' | 'resume_last' | 'run_task';
  value?: string;
  workflowId?: string;
}
...
interface PersistedPlanHistoryEntry extends PersistedPlanState {
  savedAt: string;
}

interface PersistedPlanHistoryState {
  entries: PersistedPlanHistoryEntry[];
}

interface PersistedNamedPlanState {
  plans: Record<string, PersistedPlanState>;
}

interface PersistedPlanTemplateState {
  templates: Record<string, PersistedPlanTemplateEntry>;
}

interface PersistedPlanTemplateEntry extends PersistedPlanState {
  defaults?: Record<string, string>;
}

function recommendWorkflowId(
  task: string,
  ...
  if (
    fairyTaleSignals.some((signal) => normalized.includes(signal)) &&
...
function extractSearchPattern(clause: string, filePath?: string): string {
  return clause
...
function parseNaturalLanguageAction(clause: string): ParsedAction | undefined {
  ...
  if (
    (normalized.includes('찾아') || normalized.includes('검색') || normalized.includes('search')) &&
...
function buildActionPlan(input: string): ParsedAction[] {
  const clauses = input
  ...
  const actions = clauses
    ...
    .filter((action): action is ParsedAction => action !== undefined);
...
function applyTemplateVariables(text: string | undefined, variables: Record<string, string>): string | undefined {
  if (!text) {
    return text;
  }
...
function instantiatePlanTemplate(
  plan: PersistedPlanState,
...
function collectTemplateVariables(plan: PersistedPlanState): string[] {
  const values = [
    plan.source,
...
export async function startRepl(): Promise<void> {
  ...
  let context = await loadProjectContext(project);
  const session = createSessionState();
  let pendingPlan: ParsedAction[] = [];
  let pendingPlanSource: string | undefined;
  const pendingPlanPath = resolve(project.rootDir, '.myagent', 'repl-plan.json');
  const pendingPlanHistoryPath = resolve(project.rootDir, '.myagent', 'repl-plan-history.json');
  const namedPlansPath = resolve(project.rootDir, '.myagent', 'repl-named-plans.json');
  const planTemplatesPath = resolve(project.rootDir, '.myagent', 'repl-plan-templates.json');
  let pendingPlanHistory: PersistedPlanHistoryEntry[] = [];
  let namedPlans: Record<string, PersistedPlanState> = {};
  let planTemplates: Record<string, PersistedPlanTemplateEntry> = {};
  const provider = createProvider(resolveOpenAIConfig(project.config));
  const eventStore = new EventStore(project);
  const approvalStore = new ApprovalStore(project);
  const runStore = new RunStore(project);
  const executor = new WorkflowExecutor(provider, undefined, eventStore, approvalStore);
  ...
  const showContext = (summary: ProjectContextSummary): void => {
  ...
  const persistPendingPlan = async (): Promise<void> => {
    if (pendingPlan.length === 0) {
      try {
        await fs.unlink(pendingPlanPath);
      } catch {
        return;
      }
      return;
    ...
    await fs.mkdir(resolve(project.rootDir, '.myagent'), { recursive: true });
    const payload: PersistedPlanState = {
      source: pendingPlanSource,
      actions: pendingPlan,
    };
    await fs.writeFile(pendingPlanPath, JSON.stringify(payload, null, 2), 'utf8');
  };

  const persistPendingPlanHistory = async (): Promise<void> => {
    await fs.mkdir(resolve(project.rootDir, '.myagent'), { recursive: true });
    const payload: PersistedPlanHistoryState = {
      entries: pendingPlanHistory,
    };
    await fs.writeFile(pendingPlanHistoryPath, JSON.stringify(payload, null, 2), 'utf8');
  };

  const persistNamedPlans = async (): Promise<void> => {
    await fs.mkdir(resolve(project.rootDir, '.myagent'), { recursive: true });
    const payload: PersistedNamedPlanState = {
      plans: namedPlans,
    };
    await fs.writeFile(namedPlansPath, JSON.stringify(payload, null, 2), 'utf8');
  };

  const persistPlanTemplates = async (): Promise<void> => {
    await fs.mkdir(resolve(project.rootDir, '.myagent'), { recursive: true });
    const payload: PersistedPlanTemplateState = {
      templates: planTemplates,
    };
    await fs.writeFile(planTemplatesPath, JSON.stringify(payload, null, 2), 'utf8');
  };

  const pushPendingPlanHistory = async (state: PersistedPlanState): Promise<void> => {
  ...
  const loadPendingPlan = async (): Promise<void> => {
    try {
  ...
  const loadPendingPlanHistory = async (): Promise<void> => {
    try {
      const raw = await fs.readFile(pendingPlanHistoryPath, 'utf8');
      const parsed = JSON.parse(raw) as PersistedPlanHistoryState;
  ...
  const loadNamedPlans = async (): Promise<void> => {
    try {
      const raw = await fs.readFile(namedPlansPath, 'utf8');
      const parsed = JSON.parse(raw) as PersistedNamedPlanState;
      if (!parsed.plans || typeof parsed.plans !== 'object') {
        return;
      }

      namedPlans = Object.fromEntries(
        Object.entries(parsed.plans).filter(([, plan]) => Array.isArray(plan.actions) && plan.actions.length > 0),
      );
    } catch {
      return;
    }
  ...
  const loadPlanTemplates = async (): Promise<void> => {
    try {
      const raw = await fs.readFile(planTemplatesPath, 'utf8');
      const parsed = JSON.parse(raw) as PersistedPlanTemplateState;
      if (!parsed.templates || typeof parsed.templates !== 'object') {
        return;
      }

      planTemplates = Object.fromEntries(
        Object.entries(parsed.templates).filter(([, plan]) => Array.isArray(plan.actions) && plan.actions.length > 0),
      );
    } catch {
      return;
    }
  ...
  const describeAction = (action: ParsedAction): string => {
  ...
  const showPendingPlanHistory = (): void => {
  ...
  const showPlanTemplate = (name: string): void => {
  ...
  const showPlanAwareSuggestions = (items: string[] = []): void => {
    if (pendingPlan.length === 0) {
  ...
  const executePlannedAction = async (action: ParsedAction): Promise<void> => {
  ...
  const executePendingPlan = async (): Promise<boolean> => {
    if (pendingPlan.length === 0) {
      return false;
    }
  ...
  const cancelPendingPlan = async (): Promise<boolean> => {
    if (pendingPlan.length === 0) {
      return false;
    }
  ...
  const listFiles = async (relativeDir = '.'): Promise<void> => {
    const targetDir = resolve(project.rootDir, relativeDir);
    try {
  ...
  const readFile = async (relativePath: string): Promise<void> => {
    const targetPath = resolve(project.rootDir, relativePath);
    try {
  ...
  const searchText = async (pattern: string): Promise<void> => {
  ...
  const writeFileFromPrompt = async (relativePath: string): Promise<void> => {
    const targetPath = resolve(project.rootDir, relativePath);
    const content = await captureMultilineInput(
      rl,
  ...
  const patchFileFromPrompt = async (relativePath: string): Promise<void> => {
    const targetPath = resolve(project.rootDir, relativePath);
    const original = await fs.readFile(targetPath, 'utf8');
    const oldText = await captureMultilineInput(
      rl,
      `Patch ${relativePath}: enter the exact text to replace`,
    );
    const newText = await captureMultilineInput(
      rl,
      `Patch ${relativePath}: enter the replacement text`,
    );
  ...
  const runProjectCommand = async (label: string, command: string, args: string[]): Promise<void> => {
    console.log(`${label}...`);
  ...
  const handleSessionAwareInput = async (trimmed: string): Promise<boolean> => {
    const normalized = trimmed.toLowerCase();

    if (
      (normalized.includes('마지막 run') || normalized.includes('last run')) &&
      (normalized.includes('이어') || normalized.includes('resume'))
    ) {
      if (!session.lastRunId) {
        console.error('No last run is recorded in session memory.');
    ...
    if (
      (normalized.includes('방금') || normalized.includes('다시') || normalized.includes('again') || normalized.includes('rerun')) &&
    ...
    if (
      (normalized.includes('아까 파일') || normalized.includes('last file') || normalized.includes('that file')) &&
    ...
    if (
      ...
      context = await loadProjectContext(project);
      pushSessionEntry(session, {
        kind: 'file',
  ...
  const handleNaturalLanguageIntent = async (trimmed: string): Promise<boolean> => {
    const normalized = trimmed.toLowerCase();
    ...
    if (
      (normalized.includes('목록') || normalized.includes('list') || normalized.includes('files')) &&
  ...
  const executeWorkflow = async (task: string, workflowId?: string): Promise<void> => {
    const recommendedWorkflowId = workflowId
      ? undefined
      : recommendWorkflowId(task, new Set(project.workflowMap.keys()));
    const targetWorkflowId =
      workflowId ?? recommendedWorkflowId ?? project.config.project.defaultWorkflow;
  ...
  const handleCommand = async (input: string): Promise<boolean> => {
    ...
    if (commandBody.startsWith('plan:import ')) {
      ...
      try {
        ...
        const importedPlans = Object.fromEntries(
          Object.entries(parsed.plans).filter(([, plan]) => Array.isArray(plan.actions) && plan.actions.length > 0),
        );
        namedPlans = {
          ...namedPlans,
